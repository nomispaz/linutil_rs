...
async fn run_app<B: Backend>(
    ...
    let (tx_output, mut rx_output) = mpsc::channel::<String>(300); // for stdout/stderr from command to UI
    let (tx_input, rx_input) = mpsc::channel::<String>(300); // for user input from UI to command stdin
            ...
            match event::read()? {
                Event::Key(key) => match key.code {
                    ...
                    KeyCode::Enter => {
                        ...
                        tx_input.send(input_to_send).await.unwrap();
                    ...
                    match app.current_screen {
                        CurrentScreen::Main => match key.code {
                            ...
                            KeyCode::Char('c') => {
                                ...
                                let handle = tokio::spawn(run_command(tx, rx, command));
